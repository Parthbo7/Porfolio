...
export const ExperiencePage = () => {
  ...
  const handleCardClick = (cardId: string) => {
    ...
    if (cardId === 'exp-gdg') {
      navigate('/experience/gdg');
    } else if (cardId === 'exp-startup') {
      navigate('/experience/bootcamp');
    } else if (cardId === 'exp-freshers') {
      navigate('/experience/freshers');
    } else if (cardId === 'exp-mechanics') {
      navigate('/experience/mechanics');
    } else if (cardId === 'exp-tpo') {
      navigate('/experience/tpo');
    } else {
      const cleanId = cardId.replace('exp-', '');
      navigate(`/experience/${cleanId}`);
    }
  };
  ...
  return (
    <div 
      ref={containerRef}
      className="w-full h-full min-h-screen overflow-y-auto no-scrollbar flex flex-col justify-between select-none relative p-6 sm:p-12 lg:p-16"
    >
