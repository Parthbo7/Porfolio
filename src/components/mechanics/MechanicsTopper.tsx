...
const LogImage = ({ src, alt, className, style }: { src: string; alt?: string; className?: string; style?: React.CSSProperties }) => {
  console.log('MechanicsTopper rendering imageSrc ->', src);
  return <img src={src} alt={alt} className={className} style={style} />;
};

interface MechanicsTopperProps {
  onBack: () => void;
}

export const MechanicsTopper = ({ onBack }: MechanicsTopperProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6 pt-32 w-full relative z-10">
        ...
        {/* ════════════════════════════════════════════
           SECTION 1: HERO (CINEMATIC MECHANICS HEADER)
        ════════════════════════════════════════════ */}
        <header className="mb-24 flex flex-col items-center text-center relative">
          ...
          >
            ...
            <span className="font-mono text-[9px] tracking-[0.45em] uppercase text-[#00CC52] font-extrabold">ACADEMIC_ACHIEVEMENT_LOG</span>
            <div className="h-[1px] w-8 bg-[#00CC52]/40" />
          </motion.div>
          ...
          <div className="relative mb-6">
            ...
            <span className="absolute -bottom-10 left-1/2 -translate-x-1/2 text-[100px] opacity-[0.02] pointer-events-none font-display font-black tracking-widest text-[#00CC52]">PRECISION</span>
          </div>
          ...
          >
            ...
            <p className="font-mono text-xs sm:text-sm tracking-[0.14em] uppercase leading-relaxed max-w-2xl text-[#8B8B8B]">
              Mastering one of engineering’s toughest foundations through analytical thinking and precision problem-solving.
            </p>
          ...
          {/* ── Floating OS Stickers (Monochrome Mechanics Accents) ── */}
          <div className="absolute inset-0 pointer-events-none z-20">
            {[
              { label: 'ENGINEERING_NODE', top: '12%', left: '4%', rotate: -5, delay: 0.1 },
              { label: 'ANALYTICAL_SYSTEM', top: '24%', right: '4%', rotate: 3, delay: 0.3 },
              { label: 'TOPPER_PROTOCOL', bottom: '8%', left: '8%', rotate: 4, delay: 0.2 },
              { label: 'STUDENT_INSPIRATION', bottom: '18%', right: '8%', rotate: -4, delay: 0.5 }
            ].map((sticker) => (
              <motion.div
                key={sticker.label}
          ...
          <div className="absolute inset-x-0 bottom-[-40px] pointer-events-none hidden sm:flex justify-center gap-12 font-mono text-[7px] text-[#8B8B8B]/40 uppercase tracking-[0.3em]">
            <span>[ SYSTEM ENGINE NOMINAL ]</span>
            <span>[ MECHANICAL SYSTEM SYNCHRONIZED ]</span>
            <span>[ ANALYTICAL DECRYPTION ACTIVE ]</span>
          </div>
        </header>

        {/* ════════════════════════════════════════════
           SECTION 2: ACADEMIC EXCELLENCE (TEXT LEFT, IMAGE RIGHT - MT1)
        ════════════════════════════════════════════ */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center pt-4">
              ...
              <div className="flex flex-col gap-6 text-left">
                <span className="font-mono text-[10px] text-[#00CC52] font-bold tracking-[0.3em] uppercase block">
                  NODE 01 // FOUNDATIONAL MASTERY
                </span>
                
                <h3 className="font-display font-black text-3xl sm:text-4xl uppercase tracking-tighter leading-none">
                  ENGINEERING MECHANICS TOPPER
                </h3>
                
                <div className="h-[1.5px] w-24 bg-[#00CC52]/40" />
                
                <p className={`font-sans text-[14.5px] leading-relaxed font-light ${themeTextMuted}`}>
                  Achieved top academic performance in Engineering Mechanics, one of the most conceptually demanding engineering subjects, through analytical precision, consistent problem-solving, and strong conceptual understanding.
                </p>
                
                <p className={`font-sans text-[14.5px] leading-relaxed font-light ${themeTextMuted}`}>
                  Mastered vectors, truss equilibrium matrices, dynamic friction kinematics, and foundational structural mechanics with consistent engineering application under timed evaluations.
                </p>

                <div className="flex flex-wrap gap-2 mt-4">
                  {["Engineering Mechanics", "Analytical Thinking", "Academic Excellence", "Precision Problem Solving"].map(tag => (
              ...
              <div className="w-full flex justify-center lg:justify-end">
                ...
                >
                  ...
                  <LogImage 
                    src={MT1} 
                    alt="Engineering Mechanics Topper" 
        ...
        {/* ════════════════════════════════════════════
           SECTION 3: STUDENT INSPIRATION (CENTERED SHOWCASE - MT2)
        ════════════════════════════════════════════ */}
        <section className="w-full max-w-5xl mx-auto mb-28 px-2 z-10 text-center">
          <div className={`relative backdrop-blur-md border rounded-md p-8 sm:p-12 overflow-hidden ${themeBgCard} shadow-2xl`}>
            ...
            <p className={`max-w-3xl mx-auto font-sans text-[14.5px] leading-relaxed mb-10 font-light ${themeTextMuted}`}>
              Shared knowledge, concepts, and motivation with juniors while encouraging strong fundamentals in engineering problem-solving and analytical thinking.
            </p>

            {/* Centered Large Showcase Panel (MT2) */}
            <div className="flex justify-center w-full">
              ...
              >
                ...
                <div className="absolute right-4 bottom-4 bg-black/60 border border-white/10 text-white px-2 py-0.5 font-mono text-[7px] tracking-[0.2em] rounded-sm shadow-md">
                  RECORD_MT2_MENTORSHIP
                </div>
              </motion.div>
            </div>

            <div className="flex justify-center flex-wrap gap-2 mt-8">
              {["Student Guidance", "Mentorship", "Knowledge Sharing", "Engineering Fundamentals"].map(tag => (
                <span key={tag} className={`font-mono text-[9px] uppercase px-3 py-1 border rounded-sm ${themeBgBadge}`}>
                  {tag}
                </span>
              ))}
        ...
        {/* ════════════════════════════════════════════
           SECTION 4: RECOGNITION ARCHIVE (CERTIFICATE SHOWCASE - MT3)
        ════════════════════════════════════════════ */}
          ...
          <div className={`relative backdrop-blur-md border rounded-md p-8 md:p-12 overflow-hidden ${themeBgCard} shadow-2xl`}>
            ...
            <div className="flex flex-col lg:flex-row gap-12 items-center">
              ...
              <div className="w-full lg:w-7/12 flex flex-col gap-6 text-left">
                <span className="font-mono text-[10px] text-[#00CC52] font-bold tracking-[0.3em] uppercase block">
                  NODE 03 // ENDORSEMENT & COMPILATION
                </span>
                ...
                <p className={`font-sans text-[14.5px] leading-relaxed font-light ${themeTextMuted}`}>
                  Received recognition for consistent academic performance, conceptual clarity, and contribution toward inspiring peers and junior students in engineering academics.
                </p>
                
                <p className={`font-sans text-[14.5px] leading-relaxed font-light ${themeTextMuted}`}>
                  This archive acknowledges the dedication to foundational precision, structural problem solving, and analytical clarity developed throughout core mechanics evaluations.
                </p>
                ...
                <div className="mt-6 p-6 border border-[#00CC52]/20 bg-[#00CC52]/5 rounded-sm relative overflow-hidden backdrop-blur-md">
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="font-mono text-[10px] sm:text-[11px] tracking-widest uppercase">
                      ENGINEERING_NODE // ANALYTICAL_SYSTEM // TOPPER_PROTOCOL
                    </div>
                  </div>
                </div>
              </div>
